...
def _get_sentinel(sentinel: str | None = None) -> str:
    if sentinel is None:
...
@pytest.mark.sklearn()
def test_to_xml_from_xml(extension):
...
@pytest.mark.sklearn()
def test_publish_flow(extension):
    ...
    flow, _ = _add_sentinel_to_flow_name(flow, None)
...
def test_publish_existing_flow(flow_exists_mock, extension):
...
@pytest.mark.sklearn()
def test_publish_flow_with_similar_components(extension):
    ...
    flow1 = extension.model_to_flow(clf1)
    flow1, sentinel = _add_sentinel_to_flow_name(flow1, None)
    flow1.publish()
    TestBase._mark_entity_for_removal("flow", flow.flow_id, flow.name)
    ...
    flow2, _ = _add_sentinel_to_flow_name(flow2, sentinel)
    flow2.publish()
    ...
    flow3 = extension.model_to_flow(clf3)
    flow3, _ = _add_sentinel_to_flow_name(flow3, sentinel)
...
@pytest.mark.sklearn()
def test_semi_legal_flow(extension):
...
def test_publish_error(api_call_mock, flow_exists_mock, get_flow_mock, extension):
...
@pytest.mark.sklearn()
def test_illegal_flow(extension):
    ...
    with pytest.raises(ValueError):
        extension.model_to_flow(illegal)


def test_nonexisting_flow_exists():
    def get_sentinel():
        # Create a unique prefix for the flow. Necessary because the flow
        # is identified by its name and external version online. Having a
        # unique name allows us to publish the same flow in each test run
...
@pytest.mark.sklearn()
def test_existing_flow_exists(extension):
    # create a flow
...
@pytest.mark.sklearn()
def test_sklearn_to_upload_to_flow(extension):
    ...
    rs.fit(X, y)
    flow = extension.model_to_flow(rs)
    ...
    flow, sentinel = _add_sentinel_to_flow_name(flow, None)
...
@pytest.mark.production()
def test_download_non_scikit_learn_flows():
