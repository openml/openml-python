...
@pytest.mark.production()
def test_list_flows_output_format():
    # We can only perform a smoke test here because we test on dynamic
    # data from the internet...
    flows = openml.flows.list_flows()
...
def test_are_flows_equal():
    ...
    with pytest.raises(ValueError):
        openml.flows.functions.assert_flows_equal(parent_flow, subflow)
    new_flow = copy.deepcopy(parent_flow)
    new_flow.components["subflow"].name = "Subflow name"
    with pytest.raises(ValueError):
        openml.flows.functions.assert_flows_equal(parent_flow, new_flow)


def test_are_flows_equal_ignore_parameter_values():
    ...
    with pytest.raises(
        ValueError,
        match=r"Flow Test: parameter set of flow differs from the parameters "
...
def test_are_flows_equal_ignore_if_older():
    paramaters = OrderedDict((("a", 5), ("b", 6)))
    parameters_meta_info = OrderedDict((("a", None), ("b", None)))
...
def test_sklearn_to_flow_list_of_lists(extension):
...
@pytest.mark.sklearn()
def test_get_flow_reinstantiate_model():
...
def test_get_flow_reinstantiate_model_no_extension():
    ...
    with pytest.raises(ValueError, match=".* flow: 10 \\(weka.SMO\\). "):
        openml.flows.get_flow(flow_id=10, reinstantiate=True)
...
@pytest.mark.production()
def test_get_flow_reinstantiate_flow_not_strict_post_1():
...
@pytest.mark.production()
def test_get_flow_reinstantiate_flow_not_strict_023_and_024():
...
@pytest.mark.production()
def test_get_flow_reinstantiate_flow_not_strict_pre_023():
...
@pytest.mark.sklearn()
def test_get_flow_id(long_version):
    if long_version:
...
def test_delete_flow(extension):
    from openml.testing import TestBase
    import hashlib
    import time
    import os
