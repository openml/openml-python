...
class TestOpenMLDataset(TestBase):
    ...
    def _remove_pickle_files(self):
        self.lock_path = os.path.join(openml.config.get_cache_directory(), "locks")  # noqa: PTH118
        for did in ["-1", "2"]:
            with lockutils.external_lock(
                name=f"datasets.functions.get_dataset:{did}",
                lock_path=self.lock_path,
            ):
                pickle_path = os.path.join(  # noqa: PTH118
                ...
                try:  # noqa: SIM105
                    os.remove(pickle_path)  # noqa: PTH107
    ...
    def _check_dataset(self, dataset):
        assert type(dataset) == dict  # noqa: E721
    ...
    def test_tag_untag_dataset(self):
        tag = f"test_tag_{random.randint(1, 1000000)}"  # noqa: S311
    ...
    def test_get_dataset_by_name(self):
        ...
        assert type(dataset) == OpenMLDataset  # noqa: E721
    ...
    def test_get_dataset_download_all_files(self):
        # openml.datasets.get_dataset(id, download_all_files=True)  # noqa: ERA001
    ...
    def test_get_dataset_cannot_access_private_data(self):
        ...
        self.use_production_server()
        self.assertRaises(OpenMLPrivateDatasetError, openml.datasets.get_dataset, 45)  # noqa: PT027

    @pytest.mark.skip("Need to find dataset name of private dataset")
    def test_dataset_by_name_cannot_access_private_data(self):
        self.use_production_server()
        self.assertRaises(OpenMLPrivateDatasetError, openml.datasets.get_dataset, "NAME_GOES_HERE")  # noqa: PT027
    ...
    def test_get_dataset_lazy_all_functions(self):
        ...
        def ensure_absence_of_real_data():
            assert not os.path.exists(  # noqa: PTH110
                os.path.join(openml.config.get_cache_directory(), "datasets", "1", "dataset.arff")  # noqa: PTH118
            )

        tag = f"test_lazy_tag_{random.randint(1, 1000000)}"  # noqa: S311
        dataset.push_tag(tag)
    ...
    def test__get_dataset_description(self):
        ...
        description_xml_path = os.path.join(self.workdir, "description.xml")  # noqa: PTH118
        assert os.path.exists(description_xml_path)  # noqa: PTH110
    ...
    def test__download_minio_file_to_directory(self):
        ...
        assert os.path.isfile(os.path.join(self.workdir, "dataset_20.pq")), (  # noqa: PTH113, PTH118
            "_download_minio_file can save to a folder by copying the object name"
        )

    def test__download_minio_file_to_path(self):
        file_destination = os.path.join(self.workdir, "custom.pq")  # noqa: PTH118
        _download_minio_file(
            source="http://data.openml.org/dataset20/dataset_20.pq",
            destination=file_destination,
            exists_ok=True,
        )
        assert os.path.isfile(file_destination), (  # noqa: PTH113
            "_download_minio_file can save to a folder by copying the object name"
        )
    ...
    def test_get_dataset_force_refresh_cache_clean_start(self):
        ...
        # Test clean start
        openml.datasets.get_dataset(2, force_refresh_cache=True)
        assert os.path.exists(did_cache_dir)  # noqa: PTH110
    ...
    def test_deletion_of_cache_dir(self):
        ...
        assert not os.path.exists(did_cache_dir)  # noqa: PTH110
    ...
    def test_deletion_of_cache_dir_faulty_download(self, patch):
        ...
        self.assertRaisesRegex(Exception, "Boom!", openml.datasets.get_dataset, dataset_id=1)  # noqa: PT027
        datasets_cache_dir = os.path.join(openml.config.get_cache_directory(), "datasets")  # noqa: PTH118
        assert len(list(Path(datasets_cache_dir).iterdir())) == 0

    @pytest.mark.test_server
    def test_publish_dataset(self):
        arff_file_path = (
            self.static_cache_dir / "org" / "openml" / "test" / "datasets" / "2" / "dataset.arff"
        )
    ...
    def test_attributes_arff_from_df_unknown_dtype(self):
        ...
        for arr, dt in zip(data, dtype, strict=False):
            df = pd.DataFrame(arr)
            err_msg = f"The dtype '{dt}' of the column '0' is not currently supported by liac-arff"
            with pytest.raises(ValueError, match=err_msg):
                attributes_arff_from_df(df)
    ...
    def test_create_dataset_numpy(self):
        ...
        assert _get_online_dataset_arff(dataset.id) == dataset._dataset, (
    ...
    def test_create_dataset_sparse(self):
        ...
        assert _get_online_dataset_arff(xor_dataset.id) == xor_dataset._dataset, (
            "Uploaded ARFF does not match original one"
        )
        assert _get_online_dataset_format(xor_dataset.id) == "sparse_arff", (
    ...
    def test_create_invalid_dataset(self):
        ...
        self.assertRaises(ValueError, create_dataset, **param)  # noqa: PT027
    ...
    def test_get_online_dataset_format(self):
        ...
        assert dataset.format.lower() == _get_online_dataset_format(dataset_id), (
    ...
    def test_get_dataset_cache_format_pickle(self):
        ...
        X, _y, categorical, attribute_names = dataset.get_data()
        assert isinstance(X, pd.DataFrame)
    ...
    def test_get_dataset_cache_format_feather(self):
        ...
        cache_dir_for_id = os.path.join(cache_dir, "datasets", "128")  # noqa: PTH118
        feather_file = os.path.join(cache_dir_for_id, "dataset.feather")  # noqa: PTH118
        pickle_file = os.path.join(cache_dir_for_id, "dataset.feather.attributes.pkl.py3")  # noqa: PTH118
        data = pd.read_feather(feather_file)
        assert os.path.isfile(feather_file), "Feather file is missing"  # noqa: PTH113
        assert os.path.isfile(pickle_file), "Attributes pickle file is missing"  # noqa: PTH113
    ...
    def test_data_edit_critical_field(self):
        ...
        for i in range(n_tries):
            ...
            try:
                ...
                shutil.rmtree(
                    os.path.join(openml.config.get_cache_directory(), "datasets", str(did)),  # noqa: PTH118
                )
...
def test_delete_dataset_not_owned(
    mock_delete, test_files_directory, test_server_v1, test_apikey_v1
):
    content_file = (
    ...
    with pytest.raises(
        OpenMLNotAuthorizedError,
        match=r"The data can not be deleted because it was not uploaded by you.",
    ):
...
def _assert_datasets_retrieved_successfully(
    dids: Iterable[int],
    with_qualities: bool = False,  # noqa: FBT002
    with_features: bool = False,  # noqa: FBT002
    with_data: bool = False,  # noqa: FBT002
):
...
def test_get_dataset_lazy_behavior(
    isolate_for_test,  # noqa: ARG001
    with_data: bool,
    with_qualities: bool,
    with_features: bool,  # noqa: ARG001, RUF100
):
...
def test_get_dataset_with_invalid_id() -> None:
    ...
    with pytest.raises(OpenMLServerNoResult, match="Unknown dataset") as e:  # noqa: PT012
...
def test_read_features_from_xml_with_whitespace() -> None:
    ...
    dict = _read_features(features_file)  # noqa: A001
...
def test_get_dataset_parquet(requests_mock, test_files_directory, test_server_v1):
    ...
    content_file = test_files_directory / "mock_responses" / "datasets" / "data_description_61.xml"
    ...
    assert os.path.isfile(dataset.parquet_file)  # noqa: PTH113
