...
def run_get(filename):
    ...
    first_time = []
    ids = pd.DataFrame(list_datasets.ix[:9,:])
    for did in ids['did']:
        start = time.time()
        #try:
        if True:
            data = datasets.get_dataset(did, cache_format='feather')
            data.get_data()
        #except:
        else:
            print("except")
...
run_get("pickle_cache.pkl")

# pd.set_option('display.max_colwidth', -1)
# df1 = pd.read_pickle("feather_cache.pkl")
# df2 = pd.read_pickle("pickle_cache.pkl")
# print(df1)
# print(df2)
