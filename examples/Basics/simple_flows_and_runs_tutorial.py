...
for test_index, y_true_i, y_pred_i, y_pred_proba_i in zip(
    ...
    predictions.append(
        openml.runs.functions.format_prediction(
            ...
            prediction=y_pred_i,
            truth=y_true_i,
            proba=dict(zip(task.class_labels, y_pred_proba_i, strict=False)), # type: ignore
        )
    )
...
knn_run = knn_run.publish()  # type: ignore
print(f"Run was uploaded to {knn_run.openml_url}")
print(f"The flow can be found at {knn_run.flow.openml_url if knn_run.flow else None}")
